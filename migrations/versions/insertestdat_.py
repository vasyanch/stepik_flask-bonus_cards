    ...
    with open('test_data/users.csv', encoding='utf8') as user_file:
        ...
        for i, row in enumerate(users):
            if i == 0:
                continue
            users_obj.append(
                f"INSERT INTO users (name, password_hash) VALUES ('{row[1]}', '{row[2]}')"
            )
    ...
    with open('test_data/stores.csv', encoding='utf8') as stores_file:
        ...
        for i, row in enumerate(stores):
            if i == 0:
                continue
            stores_obj.append(
                f"INSERT INTO stores (name, address, password_hash) VALUES ('{row[1]}', '{row[2]}', '{row[3]}')"
            )
    ...
    with open('test_data/cards.csv', encoding='utf8') as cards_file:
        ...
        for i, row in enumerate(cards):
            if i == 0:
                continue
            cards_obj.append(
                f'INSERT INTO cards (bonus, level, user_id) VALUES ({row[1]}, {row[2]}, {row[3]})'
            )
